...
class BracketMatcherView {
  ...
  updateMatch () {
    ...
    let matchPosition
    let {position, currentBracket, matchingBracket} = this.findCurrentPair(false)
    if (position) {
      matchPosition = this.findMatchingEndBracket(position, currentBracket, matchingBracket)
    } else {
      ({position, currentBracket, matchingBracket} = this.findCurrentPair(true))
      if (position) {
    ...
    this.editor.transact(() => {
      ...
      if (this.matchManager.pairedCharacters.hasOwnProperty(text) || this.matchManager.pairedCharactersInverse.hasOwnProperty(text)) {
        ...
        if (position && matchPosition) {
          ...
          if ((position.row === matchPosition.row) && this.matchManager.pairedCharactersInverse.hasOwnProperty(currentBracket)) {
  ...
  findCurrentPair (isInverse) {
    ...
    let currentBracket = this.editor.getTextInRange(Range.fromPointWithDelta(position, 0, 1))
    if (!matches[currentBracket]) {
      ...
      currentBracket = this.editor.getTextInRange(Range.fromPointWithDelta(position, 0, 1))
    }
    const matchingBracket = matches[currentBracket]
    if (matchingBracket) {
      return {position, currentBracket, matchingBracket}
    } else {
      return {}
    }
  }
  ...
  selectInsideBrackets () {
    ...
    if (this.pairHighlighted) {
      ...
      startPosition = this.findPrecedingStartBracket(this.editor.getCursorBufferPosition())
      if (startPosition) {
